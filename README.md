<h1 align="center">
    Hi,<br>
    Fancyflame is here.<br>
    Welcome!  👋
    <br>
    <br>
    <img src="https://img.shields.io/badge/rust-%23000000.svg?style=for-the-badge&logo=rust&logoColor=white">
    <img src="https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E">
    <img src="https://img.shields.io/badge/stylus-%23ff6347.svg?style=for-the-badge&logo=stylus&logoColor=white">
    <br>
    <img src="https://img.shields.io/badge/vuejs-%2335495e.svg?style=for-the-badge&logo=vuedotjs&logoColor=%234FC08D">
    <img src="https://img.shields.io/badge/node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white">
    <img src="https://img.shields.io/badge/Visual%20Studio%20Code-0078d7.svg?style=for-the-badge&logo=visual-studio-code&logoColor=white">
    <br>
    <img src="https://img.shields.io/badge/Minecraft-171615.svg?style=for-the-badge&logoColor=white&logo=data:image/png;base64,">
    <br>
    <div>
        <code>Rust教徒</code>
        <code>宏法师</code>
        <code>Minecraft玩家</code>
        <code>语言洁癖</code>
    </div>
    <br>
    <img width="300" align="center" src="https://github-readme-stats.vercel.app/api/top-langs/?username=fancyflame&hide=python">
    <img width="500" align="center" src="https://github-readme-stats.vercel.app/api?username=fancyflame&show_icons=true&theme=monokai">
    <br><br><br>
</h1>

### My Repositories

<table align="center">
    <tr>
        <td width="400">
            <a href="https://github.com/fancyflame/irisia-gui"><img src="https://github-readme-stats.vercel.app/api/pin/?username=fancyflame&repo=irisia-gui"></a>
        </td>
        <td>
        <b>Irisia GUI</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br>
        Irisia GUI是一个基于Rust语言开发的响应式跨平台GUI框架，拥有事件系统、异步运行时等组件。正在开发中。
        </td>
    </tr>
    <tr>
        <td>
            <a href="https://github.com/fancyflame/mcsh"><img src="https://github-readme-stats.vercel.app/api/pin/?username=fancyflame&repo=mcsh"></a>
        </td>
        <td>
            <b>MCSH</b><br>
            MCSH是一个类似Rust语法的编译型语言，编译目标为Minecraft指令文件（mcfunction文件），可打包为mcpack。在游戏内可通过<code>/function</code>指令使用标记导出的函数。项目有完整的语法解析和编译流程。
        </td>
    </tr>
    <tr>
        <td>
            <a href="https://github.com/fancyflame/mcmu-rs"><img src="https://github-readme-stats.vercel.app/api/pin/?username=fancyflame&repo=mcmu-rs"></a>
        </td>
        <td>
            <b>MCMU系列</b><br>
            MCMU项目系列是为Minecraft做的远程联机工具。开启房间后，远程玩家可通过输入房间号来加入你的房间。当你进入世界时，远程玩家可以在游戏里看到你的信号并加入一起游玩。
        </td>
    </tr>
    <tr>
        <td>
            <a href="https://github.com/fancyflame/shadowskin"><img src="https://github-readme-stats.vercel.app/api/pin/?username=fancyflame&repo=shadowskin"></a>
        </td>
        <td>
            <b>ShadowSkin系列</b><br>
            ShadowSkin系列是为Minecraft做的基于网页的皮肤制作器。自ShadowSkin2完成后现在项目长期暂停。
        </td>
    </tr>
</table>

